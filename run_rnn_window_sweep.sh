...
RESULTS_ROOT="${RESULTS_ROOT:-$ROOT_DIR/outputs_prediction_DOY/models/harvest_rnn_window_sweep_2022_train_2023_test}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/harvest_rnn_window_sweep_2022_train_2023_test}"
EXCEL_PATH="${EXCEL_PATH:-$ROOT_DIR/harvest Prediction RNN.xlsx}"
WANDB_DIR="${WANDB_DIR:-$ROOT_DIR/wandb}"
WANDB_PROJECT="${WANDB_PROJECT:-DeepSatModels-rnn-window-sweep}"
...
if [[ -z "${WANDB_API_KEY:-}" ]]; then
  echo "WANDB_API_KEY is not set. Export it before running this script." >&2
  exit 1
fi
  ...
  for FEATURE_SET in "${FEATURE_SETS[@]}"; do
    for CROP in "${CROPS[@]}"; do
      ...
      CMD=(
        ...
        --wandb-tags rnn_window_sweep train2022 test2023 "$WINDOW_NAME" "$FEATURE_SET" "$CROP"
      )
...
"$PYTHON" -m doy_prediction.aggregate_rnn_window_results \
  --results-root "$RESULTS_ROOT" \
  --out-xlsx "$EXCEL_PATH" \
  --status-tsv "$STATUS_TSV"
AGG_EXIT_CODE=$?
if [[ "$AGG_EXIT_CODE" -ne 0 ]]; then
  echo "Aggregation failed with exit_code=$AGG_EXIT_CODE" >&2
  exit "$AGG_EXIT_CODE"
fi

echo "Excel summary: $EXCEL_PATH"
exit "$ANY_FAILED"
